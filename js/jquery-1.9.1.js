...
!== null ) {
	...
	propHooks: {
		tabIndex: {
			get: function( elem ) {
				// elem.tabIndex doesn't always return the correct value when
...
boolHook = {
	get: function( elem, name ) {
		var
			// Use .prop to determine if this attribute is understood as
			// boolean
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	support.getByClassName = assert(function( div ) {
		...
		div.innerHTML = "<div class='hidden e'></div><div
class='hidden'></div>";
...
"(" + whitespace + "|$)" )) &&
				classCache( className, function( elem ) {
					return pattern.test( elem.className || (typeof
...
function select( selector, context, results, seed ) {
	...
	if ( !seed ) {
		...
		if ( match.length === 1 ) {
			...
			// Take a shortcut and set the context if the root selector is an
			// ID
...
node.textContent || node.innerHTML || "" ).replace( rcleanScript, "" ) );
							}
						}
					}
				}
...
var xhrCallbacks, xhrSupported,
	xhrId = 0,
	// #5280: Internet Explorer will keep connections alive if we don't abort
	// on unload
		...
		"*": [function( prop, value ) {
			...
			if ( parts ) {
				...
				if ( unit !== "px" && start ) {
					...
					// Prefer the current property, because this process will
					// be trivial if it uses the same units
