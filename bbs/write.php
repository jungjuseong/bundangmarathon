 ...
 **************************************************************************/
	...
	if(!eregi("zboard.php",$HTTP_REFERER) && !eregi("view.php",$HTTP_REFERER)) Error("정상적으로 글을 쓰시기 바랍니다","window.close");
...
echo "$setup[grant_write]<$member[level]&&!$is_admin";
...
// 수정 글일때 권한 체크
	if($mode=="modify"&&$data[ismember]) {
		if($data[ismember]!=$member[no]&&!$is_admin&&$member[level]>$setup[grant_delete]){ 
			Error("사용권한이 없습니다","login.php?id=$id&page=$page&page_num=$page_num&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&no=$no&file=zboard.php");
		}
	}
