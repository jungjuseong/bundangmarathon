...
//	$dir = "skin/$setup[skinname]";
  ...
  if(!$s_data[child]) // 답글이 없을때;;
  {
   ...
   if($m_data) {
	   ...
	   //레볼루션 게시물 데이터 삭제
	   mysql_query("delete from dq_revolution where zb_id='$id' and zb_no='$no'") or Error(mysql_error()); 
   }
   ...
   // 회원일 경우 해당 해원의 점수 주기
   if(!$skin_setup[write_nopoint] && $member[no]==$s_data[ismember]) @mysql_query("update $member_table set point1=point1-1 where no='$member[no]'",$connect) or error(mysql_error());
  }
  ...
  movepage("zboard.php?id=$id&page=$page&page_num=$page_num&select_arrange=$select_arrange&desc=$des&sn=$sn&ss=$ss&sc=$sc&su=$su&keyword=$keyword&sn1=$sn1&divpage=$divpage");
?>
