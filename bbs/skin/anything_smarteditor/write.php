...
<table border=0 width=750 cellsapcing=0 cellpadding=0>
<tr><td>
<form method=post name=write action=write_ok.php onsubmit="submitContents(); return check_submit();" enctype=multipart/form-data>
...
<table border=0 width=720 cellsapcing=0 cellpadding=0 cellspacing=0>
  <tr> 
    <td height=17 width=1 align=left valgin=middle background=<?=$dir?>/robinweb_topbar_both.gif></td>
    <td align=left valgin=middle background=<?=$dir?>/robinweb_topbar.gif><?
    ...
    ?></td>
    <td width=1 align=left valgin=middle background=<?=$dir?>/robinweb_topbar_both.gif></td>
  </tr>
</table>
<table border=0 width=720 cellsapcing=1 cellpadding=0 height=1 cellspacing=0><tr><td></td></tr></table>
<table border=0 width=720 cellsapcing=1 cellpadding=0 class=robin_writeform>
...
<tr>
  <td align=left>
	<img src=<?=$dir?>/t.gif border=0 height=1><br><img src=<?=$dir?>/w_select.gif>
	<td>
       <?=$category_kind?>
       <?=$hide_notice_start?> <input type=checkbox name=notice <?=$notice?> value=1> 공지사항 <?=$hide_notice_end?>
       <?=$hide_html_start?> <input type=hidden name=use_html  value=2 checked><?=$hide_html_end?>
...
<tr valign=top>
  <td width=10%><img src=<?=$dir?>/t.gif border=0 height=1><br><img src=<?=$dir?>/w_subject.gif></td>
  <td><input type=text name=subject value="<?=$subject?>" <?=size(50)?> maxlength=300 style=width:95% class=robin_input>
  </td> 
</tr>
<tr>
...
<div id="counterDiv">
	<span id="num_loaded">0</span> / <span id="queued_total">0</span> 개의 사진이 로드됨 (여러개의 사진을 한꺼번에 올릴 수 있습니다.)
</div>
...
<div id="statudDiv">&nbsp;</div>
<div>
    <span id="spanButtonPlaceHolder"></span> 
		<input id="undercover" type = "button" value="사진올리기"/>
		<input id="btnCancel" type="button" value="취소" onclick="swfu.cancelQueue();" disabled="disabled" />
</div>
</td>
</tr>
<tr>
	<td colspan=2 style=padding:10px>
		<textarea name="memo" id="memo" rows="10" cols="50" style="width:690px;height:300px;display:none;">
			<? echo $memo ?>
		</textarea>
	</td>
</tr>
