 ...
 ******************************************************************************/
	...
	if(!eregi("install",$PHP_SELF)&&file_exists($_zb_path."config.php")) {
		...
		if(!is_writable($_zb_path."data")) 
			error("Data 디렉토리의 쓰기 권한이 없습니다<br>제로보드를 사용하기 위해서는 Data 디렉토리의 쓰기 권한이 있어야 합니다");
		if(!is_writable($_zb_path."icon")) 
			error("icon 디렉토리의 쓰기 권한이 없습니다<br>제로보드를 사용하기 위해서는 icon 디렉토리의 쓰기 권한이 있어야 합니다");
		if(!is_writable($_zb_path.$_zbDefaultSetup[session_path])) 
			error("세션 디렉토리(".$_zb_path.$_zbDefaultSetup[session_path].")의 쓰기 권한이 없습니다<br>제로보드를 사용하기 위해서는 세션디렉토리의 쓰기 권한이 있어야 합니다");
		...
		//filewrite( "lib.php 1 HTTP_SESSION_VARS[zb_logged_no]=".$HTTP_SESSION_VARS["zb_logged_no"]." ".date("Y-m-d H:i:s"));
		//echo( "<!-- lib.php 1 HTTP_SESSION_VARS[zb_logged_no]=".$HTTP_SESSION_VARS["zb_logged_no"]." zbSessionID=".$HTTP_COOKIE_VARS[ZBSESSIONID]."-->\n");
		unset($autoLoginData);
...
//echo "no";
		...
		if($member_data[user_id] == 'run4joy' || $HTTP_SESSION_VARS[zb_logged_ip]=="211.195.221.204"){
			echo "<br>lib1.php<br>";
			echo "autoLoginData[no]=".$autoLoginData[no]."<br>";
			echo ",time=".time()." zb_logged_time=".$HTTP_SESSION_VARS["zb_logged_time"]." defaultSetup[login_time]=".$_zbDefaultSetup["login_time"]." zb_logged_ip=".$HTTP_SESSION_VARS["zb_logged_ip"]." remote_addr=".$REMOTE_ADDR;
			echo ", zb_logged_no=".$HTTP_SESSION_VARS["zb_logged_no"];
			echo "<br>lib1.php end<br>";
			exit(0);
		}
	...
	if(eregi("mobile",$HTTP_USER_AGENT)) $browser="0";	// yhkim
...
//echo "dir.no.gif=".$dir.$no.".gif";
	...
	function get_face($data, $check=0) {
		...
		// yhkim(2 lines)
//		$photoname=mysql_fetch_array(mysql_query("select photo from member,zetyx_member_table where userid=user_id and no=".$data[ismember]));
//		if(@file_exists("../member/photo/".$photoname[photo]) && strlen($photoname[photo])>0)
//			$face_image= "../member/photo/".$photoname[photo];
//echo "face_image=".$face_image." photo=".$photoname[photo]." ".$data[ismember]." group[use_icon]=".$group[use_icon];
		return $face_image;
	}
...
function head($body="",$scriptfile="") {
	...
	//echo("<!-- licence text in bbs/lib.php -->\n");
	//$f = @fopen("license.txt","r");
	//$license = @fread($f,filesize("license.txt"));
	//@fclose($f);
	//print "<!--\n".$license."\n-->\n";

	if(!eregi("member_",$PHP_SELF)) 
		$stylefile="skin/$setup[skinname]/style.css"; 
	else 
		$stylefile="style.css";
...
<?
	...
	if ($scriptfile) {
		...
		//echo("<script type='text/JavaScript' src='script/".$scriptfile."'></script>\n");
	}
?>
<?

	if(!($PHP_SELF=="/bbs/view.php" && $dispmode=="contonly"))	// yhkim
		if($group[header_url]) { 
	...
	if(!($PHP_SELF=="/bbs/view.php" && $dispmode=="contonly"))	// yhkim
		if($group[header]) 
			echo stripslashes($group[header]);

	if($PHP_SELF=="/bbs/view.php" && $dispmode=="contonly"){	// yhkim
		echo "<br><p align=center>".$setup[title]."\n";
	} 
	else if($setup[header]) 
		echo stripslashes($setup[header]);
?>

<table border=0 cellspacing=0 cellpadding=0 width=<?=$width?> height=1 style="table-layout:fixed;">
	<col width=100%></col>
	<tr><td><img src=images/t.gif border=0 width=98% height=1 name=zb_get_table_width><br>
	<img src=images/t.gif border=0 name=zb_target_resize width=1 height=1></td></tr>
</table>
<?
	} 
	else {
...
function foot() {
	...
	global $PHP_SELF, $dispmode; // yhkim
...
<?
	...
	if($group[footer]) 
		echo stripslashes($group[footer]);
	if(!($PHP_SELF=="/bbs/view.php" && $dispmode=="contonly"))
		if($group[footer_url]) { @include $group[footer_url]; }
...
//yhkim
		if($id=="noticeboard" || $id=="memboard")
			$memno = $data[isnumber];
		else
			$memno = "";
//		$zbLayer = $zbLayer."\nprint_ZBlayer('zbLayer$_zbCheckNum', '$data[homepage]', '$data[email]', '$data[ismember]', '$id', '$data[name]', '$traceID', '$traceType', '$isAdmin', '$isMember');";
		$zbLayer = $zbLayer."\nprint_ZBlayer('zbLayer$_zbCheckNum', '$data[homepage]', '$data[email]', '$memno', '$id', '$data[name]', '$traceID', '$traceType', '$isAdmin', '$isMember');";
	}
	return $_zbCount;
}
...
//echo "check_blockip $REMOTE_ADDR count=$count avoid_ip=$avoid_ip[0] ************";
		for($i=0;$i<$count;$i++) {
			if(!isblank($avoid_ip[$i])&&eregi($avoid_ip[$i],$REMOTE_ADDR)) Error("차단당한 IP 주소입니다.");
		}
//if(eregi("147.6.", $REMOTE_ADDR) ) exit("ip_block exit");
	}
...
filewrite( "lib.php 1 getZBSessionID() str path/zbSessionID=".$_zb_path.$_zbDefaultSetup[session_path]."/zbSessionID_".$zbSessionID.".php");
//echo "zbSessionID=".$zbSessionID;
		if(!$zbSessionID) return "";
		$str = zReadFile($_zb_path.$_zbDefaultSetup[session_path]."/zbSessionID_".$zbSessionID.".php");
filewrite( "lib.php 1 getZBSessionID() str path=".$_zb_path.$_zbDefaultSetup[session_path]."/zbSessionID_".$zbSessionID.".php");
//echo "str=".$str;
		if(!$str) {
		...
		if($member_data[user_id] == 'run4joy'){
			echo "zbSessionID=".$zbSessionID;
			echo ",zbSessionFilePath=".$_zb_path.$_zbDefaultSetup[session_path]."/zbSessionID_".$zbSessionID.".php";
			echo "exit point is lib.php getZBSessionID";
			exit(0);
		}
		return $data;
	}
	...
	function getDefaultSetup() {
		...
		if(!$defaultSetup[login_time]) $defaultSetup[login_time] = 60*1800;	// yhkim 30 -> 180
...
function c_picture_member($member_no, $c_picture){
	include_once "bmconfig.php";
	$dbquery="SELECT member.photo,member.name FROM zetyx_member_table,member where zetyx_member_table.no = $member_no and zetyx_member_table.user_id=member.userid";
	$result = mysql_query($dbquery)  or die("mysql_query error(member table )");
	$row = mysql_fetch_array($result);
//echo "xx $path2photo/$row[0] xx ";
//	if(file_exists('$path2photo/$row[0]')) // 왜 작동 안 하는지 아무리 봐도 모르겠음
		 $c_pic="<img src='/member/photo/$row[0]' width=100 border=0 alt='$row[1] photo'>";
//echo "picture(y)=".$c_picture;
	return $c_pic;
}
...
function filewrite($msg){
	global $_zb_path;
		...
		$f = fopen($_zb_path."/tmp/msgtmp.txt","a");
		$lock=flock($f,2);
		if($lock) {
			fwrite($f,"$msg\n");
			fflush($f);
		...
		flock($f,3);
		fclose($f);
		chmod ($_zb_path."/tmp/msgtmp.txt", 0777);
}
