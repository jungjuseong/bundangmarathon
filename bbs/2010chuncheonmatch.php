...
require("bmfunction.php");
	...
	top("");
	heading("2010 춘천마라톤대회 빅매치");

	if($logid == ""){
		die("로그인하지 않았습니다.");
	}
if($mode != "pub"){
	$query_name="pollid,userid,polltime,poll0,poll1,poll2,poll3,poll4,poll5,poll6,poll7,poll8";
	$query_value="'".$pollid."','".$logid."',now(),'".$rule0."','".$rule1."','".$rule2."','".$rule3."','".$rule4."','".$rule5."','".$rule6."','".$rule7."','".$rule8."'";
...
if($logid == "seosc" || $logid == "run4joy"){
	...
	echo "<p> <font color=blue> 베팅 결과는 10월 25일(월요일) 공개됩니다.</font>";
}
?>
</center>
</body>
