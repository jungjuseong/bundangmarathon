...
// 게시판 $id 체크
	if(!$id&&$_zboardis) error_m("게시판 이름을 지정해 주셔야 합니다.<br><br>예) zboard.php?id=이름",""); // 게시판 이름 체크
 ...
 **************************************************************************/
    ...
    if(!$setup[name]) error_m("생성되지 않은 게시판입니다.<br><br>게시판을 생성후 사용하십시요",""); // 설정되지 않은 게시판
		...
		if($group[is_open]==0&&!$is_admin&&$member[group_no]!=$setup[group_no]) error_m("공개 되어 있지 않습니다");
		...
		if($setup[use_category]) {
			...
			$_dbTime += getmicrotime()-$_dbTimeStart;
			$a_category="<select name=category id='selectCategory'><option value=''>카테고리</option>";
