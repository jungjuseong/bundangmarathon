...
<?Start();?></td>
              ...
              <tr> 
                <td align="center"> 
                <form method=post name=PassForm action='./gdantispamout.php'>  
...
function IpTest()
{
 	...
 	$RTime=fread($im,10);
 	fclose($im);
//echo "<script>alert('$REMOTE_ADDR');</script>"; 	
...
function DelBmp()
{
	$d=opendir("./antispambmp");
 	...
 	while($FileName=readdir($d)) {
 		...
 		if($MTime[1]-$resName[1]>120 && $FileName != ".") 
 		{
...
function Start()
{
	...
	$Pass[2]=(rand(0,9));
	$Pass[3]=(rand(0,9));
	$PassCode=$Pass[0].$Pass[1].$Pass[2].$Pass[3];
	...
	$str="./antispambmp/".$NowTime.".png";
 	include("gdmakeimg.php");
	MakeImg($Pass,$str);
	
 	$im=fopen("./antispambmp/".$NowTime.".php","w");
 	fwrite($im,"<?//",4);
 	fwrite($im,$PassCode,4);
 	fwrite($im,'_',1);
 	fwrite($im,$UniqId);
 	fwrite($im,"?>",2);
 	fclose($im);
 	echo("<img src='./antispambmp/${NowTime}.png' width=96 height=50 border=0>");
	chmod("./antispambmp/".$NowTime.".php",0700);
