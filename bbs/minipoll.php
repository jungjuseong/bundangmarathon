...
<style type='text/css'>
<!--
font{ font-size: 9pt; line-height:17px; }
-->
</style>
</head>\n";
...
$yyyymmdd=$yyyy."-".$mm."-".$dd;
$tomorrow = date("Y-m-d", mktime(0,0,0,$mm,$dd+1,$yyyy));
...
//echo $dbquery;
	...
	if($qno == 0){
		...
		echo "<div style='color:red; text-decoration : blink'>★★★★★ New ★★★★★</div><form action='$PHP_SELF' method=post>\n<input type=hidden name=mode value='poll'>\n<input type=hidden name=pollno value='$pollno'>\n<input type=hidden name=items value='$row[3]'>";
		$polldays = substr($row[start_date], 5);
		if($row[start_date]!=$row[stop_date])
			$polldays .= "~".substr($row[stop_date], 5);

		$dbquery="select name from member where userid='$row[userid]'";
		$result3 = mysql_query($dbquery) or die("mysql_query error(member table select)");
		$row3 = mysql_fetch_array($result3);
		$pollmaker = $row3[name];

		echo "$pollno. $row[question] [$pollmaker,$polldays]\n";
		$i = 0;
		$checked = "";
		if(trim($row[answers]) != ""){
		...
		echo "<br><input type=submit value='투표하기'>";
		echo "</form>\n";
...
//	if($logid == "run4joy")
		echo " <a href='$PHP_SELF?mode=input'>신규등록</a>";
...
//	if($logid == "run4joy")
	...
	echo "<미니투표 등록><br><!-- 설문 등록은 회원이면 누구나 가능합니다.-->제목,설문항목,날짜를 정확히 입력 바랍니다.\n";
	echo "<form action='$PHP_SELF' method=post>\n<input type=hidden name=mode value='insert'>\n";
	echo "시작일:<input type='text' name='start_date' size=10 value='$yyyymmdd'>\n";
	echo "<br>종료일:<input type='text' name='stop_date' size=10 value='$tomorrow'>\n";
	echo "<br>성격:<input type=checkbox name='must' value='Y'>체크시 의무\n";
	echo "<br>제목:<br><textarea name='question' cols=15 rows=2 wrap=auto>(최대 80자까지)</textarea>\n";
	echo "<br>설문항목:\n";
	for($i = 0; $i < $answeritems; $i++){
	    echo "<br>".($i+1).". <input type='text' name='pollano[$i]' size=15>\n";
	}
...
//				$etc .= "-".mb_convert_encoding($rowa[etc],"UTF-8","EUC-KR")."<br>";
		...
		if($ano == 0){
			...
			if($userid == $logid || $logid == "run4joy"){
				echo " <a href='$PHP_SELF?mode=delete&pollno=$pollno'>삭제</a><br>\n";
			}
			polllist();
			exit;
		}
