<?
  ...
  {
//yhkim(2005.12.25) : 바로 아래 if문 TRUE -> 다른 사람이 comment 단 경우 삭제 불가
$total=mysql_fetch_array(mysql_query("select count(*) from $t_comment"."_$id where parent='$s_data[no]' and name!='$s_data[name]'"));
if($total[0] > 0){	
	Error("다른 사람이 단 코멘트가 $total[0]개 있어 삭제할 수 없습니다");
}else{
