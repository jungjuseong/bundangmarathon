...
global $member, $name, $id, $no, $REMOTE_ADDR, $memo;
	...
	if($lock) {
		fwrite($f,"\n*******************************************\n");
		fwrite($f,"$member[user_id] $name $id $REMOTE_ADDR ".date("Y-m-d H:i:s")."\n");
...
// 각종 변수 검사;;
	...
	if(isblank($memo) && $id!="m_letter") Error("내용을 입력하셔야 합니다");
 ...
 **************************************************************************/
		...
		$no=mysql_insert_id();
		if($is_4989=="1")
			mysql_query("insert into buynsell ( boardid, boardno, commentno, ismember, buyer, wanttopayday, payday, price, etc) values ('$id', $no, 0, '$member[no]', '', '', '', 0, '')") or error(mysql_error()); // yhkim
