...
// end : 하루 한번씩 처리하는 일들. one time in a day
	 ...
	 ******************************************************/
	 ...
	 function print_bbs($skinname, $title, $id, $num=50, $textlen=250, $datetype="y-m-d") {
		global $_zb_path, $_zb_url, $connect, $t_board, $admin_table;

		if(!$skinname||!$id||!$title) 
			return;

		$skin = zReadFile($_zb_path."latest_skin/".$skinname."/main.html");
		if(!$skin) {
		...
		while($data=mysql_fetch_array($result)) {
			...
			$subject = cut_str21(stripslashes($data[subject]),$textlen);
			...
			$main = $loop;
			$main = str_replace("[name]",iconv("EUC-KR","UTF-8",$name),$main);
			$main = str_replace("[memo]",iconv("EUC-KR","UTF-8",$memo),$main);
			$main = str_replace("[date]",$date,$main);
			$main = str_replace("[subject]",iconv("EUC-KR","UTF-8",$subject),$main);
