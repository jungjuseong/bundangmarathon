...
//yhkim start
		...
		filewrite("admin/admin_exec_member.php $member[user_id] $member[name] ".date("Y-m-d H:i:s"));
//yhkim end
...
// 회원 권한 변경하는 부분
	...
	if($exec2=="moveall") {
		for($i=0;$i<sizeof($cart);$i++) {
//yhkim 2007.8.18
			$member_tmp=mysql_fetch_array(mysql_query("select * from $member_table where no='$cart[$i]'"));
			
			mysql_query("update $member_table set level='$movelevel' where no='$cart[$i]'",$connect);

//yhkim 2007.8.18
			if($movelevel != $member_tmp[level]) {
				if($level==5)
					$membertype="정회원";
				else if($level==6)
					$membertype="준회원";
					...
					$membertype="비정상회원";
				$que = "update member set membertype='$membertype'";
				$que.=",indate=concat('".date("Y/m/d").":$membertype;',indate)";
				$que.=" where userid='$member_tmp[user_id]'";
				@mysql_query($que) or Error("회원등급 수정시에 에러가 발생하였습니다 ".mysql_error());
			}
//
		}
...
//yhkim
		$member_tmp=mysql_fetch_array(mysql_query("select * from $member_table where no='$member_no'"));
		...
		$que.=",comment='$comment'";
		$que.=",open_picture='$open_picture'";	// yhkim insert 2005.07.24
		$que.=" where no='$member_no'";

		@mysql_query($que) or Error("회원정보 수정시에 에러가 발생하였습니다 ".mysql_error());
//yhkim
		$que="update member set name='$name'";
		if($level && $level != $member_tmp[level]) {
			if($level==5)
				$membertype="정회원";
			else if($level==6)
				$membertype="준회원";
				...
				$membertype="비정상회원";
			$que.=",membertype='$membertype'";
			$que.=",indate=concat('".date("Y/m/d").":$membertype;',indate)";
		}
		if($password&&$password1&&$password==$password1) $que.=" ,passwd='$password'";
		$que.=",email='$email'";
		$que.=",orghref='$homepage'";
		$que.=",org='$job'";
		$que.=",postaddr='$home_address'";
		$que.=",telhome='$home_tel'";
		$que.=",teloffice='$office_tel'";
		$que.=",telhand='$handphone'";
		$que.=",etc='$comment'";
		$que.=" where userid='$member_tmp[user_id]'";
		@mysql_query($que) or Error("회원등급 수정시에 에러가 발생하였습니다 ".mysql_error());
//
