...
?>
                    </select>정회원:5, 준회원:6, 휴면회원:7, OB회원:8, 확인중:9, 가입시:10</td><!-- yhkim -->
  </tr>
...
<? if($group_data[use_picture]) { ?>
  <tr height=22 align=center>
     ...
     <td align=left bgcolor=#e0e0e0>&nbsp;<input type=file name=picture size=37 maxlength=255 class=input style=border-color:#b0b0b0> (크기는 200*200 이하)
                 <? if($member_data[picture]) echo"<br>&nbsp;<img src='$member_data[picture]' border=0>"; ?>
                 <input type=checkbox name=open_picture value=1 <?=$check[$member_data[open_picture]]?> <!-- // yhkim insert 2005.0724 -->
     </td>
  </tr>
<? } ?>
