...
//    $logid=$PHP_AUTH_USER;
    ...
    if($mode == "submenu"){
		...
		$result = mysql_query($dbquery) or die("mysql_query error");
		if($row=mysql_fetch_array($result)){
			if( $row[0]!='정회원' && $row[0]!='준회원' && $row[0]!='임시'){
				echo "<br><br><br>회원광장은 정회원과 준회원만 이용할 수 있습니다.(framemenu,id='$logid',select error)";
				return;
			}else{
				include("bmmenu.php");
			}
		}else{
			echo "<br><br><br>회원광장은 정회원과 준회원만 이용할 수 있습니다.(framemenu,id='$logid')";
			return;
		}
    }else if($mode == "framemenu_public"){
		include("bmmenu_public.php");
/*
    }else if($mode == "hanmir"){
...
{ 
	window.open('./gdantispamin.php?UniqId='+id,'AntiSpam','width=180,height=263,left=1,top=1,resize'); 
} 
</script>\n";
...
//echo "dbquery=$dbquery ";
	...
	if($row=mysql_fetch_array($result)){
		...
		if($photo != ""){
			if(is_file($path2photo."/$photo"))
				echo "<p align=center><img src='".substr($path2photo, strlen($home))."/$photo' border=0>\n";
			else
	...
	$dbquery .= " where inviting='Y' and (userid='$logid' or replace(raceday, '/', '-') >= current_date) and raceday > '". date("Y/m/d"). "' order by raceday";
...
//echo "dbquery=$dbquery";
	...
	while($row=mysql_fetch_array($result)){
		...
		if($rows == 0){
			...
			if(substr($race, 0, 1) == " ")
				$race=substr($race, 1);
			echo "<br><a href='bminviting.php?mode=inviting-input2&raceid=$row[0]'>$race($ymd)</a>\n";
		}
	}
	echo "</font>\n";
	mysql_free_result($result);
...
//	echo "<p><font style='font:bold;'>일정 표시</font>\n";
	...
	if(nextimg($name) == 0){
		echo "<p align=center style='color:red;'>대회 참가 사진이 없습니다.<br>좋은 사진을 골라 올려 주십시오.\n";
	}
	...
	if($req == "yes"){
		echo "<a href='bmmem.php?mode=member-change'>본인정보수정</a><br><br>";
	}
	echo "</font>";
	echo "<a href='bmrecord.php?mode=record-input'>본인기록등록</a>";
...
//echo $dbquery;
	...
	/* outlogin.php에 복사 사용됨 */
	echo "<p><font style='font:bold;'>회원 변동<br>(한달간)</font><br>\n";
	...
	$dbquery .= " from member where (to_days(now())-to_days(indate))<=31 order by typeorder, idt desc";
...
//echo $dbquery;
	...
	while($row=mysql_fetch_array($result)){
		...
		$inyymd = split(";", $row[1]);
		$newname = $row[0]."(".substr($inyymd[0],5).")";
		echo "<a href='bmmeminfo.php?mode=meminfo-one&userid=$row[2]'>$newname</a><br>\n";
	}
	mysql_free_result($result);
	echo "</td></tr></table>\n";


	echo "<br><hr color=red width='80%'><br>회원광장 문의는 <a href=\"javascript:AntiSpam('run4joy')\">여기로</a>";
}
...
<br>
ID/Password를 모르시는 분은 san2run@paran.com으로 메일 보내 주십시오.
-->
...
<p>
<font size='+1'><a href='member_join.php?id=memboard' target=bbs>** 회원 가입 신청 **</a></font>
<br>
</td><tr>
</table>
...
function logon(){
    header( 'WWW-Authenticate: Basic realm="분당마라톤(정회원,준회원용)		*예비회원은 사용할 수 없습니다.		*사용자 이름에 사용자 ID를 사용하십시오."' );
...
//echo "logid=$logid\n";
	if($row=mysql_fetch_array($result)){
		if( $row[0]=='정회원' || $row[0]=='준회원' || $row[0]=='임시'){
			if($url == ""){
				$cont = "$PHP_SELF?mode=framecont";
			}else{
				$cont = base64_decode($url);
			}
//echo "cont=$cont";
			echo "
<html>
...
<frameset rows='40, *, 40' border='0' frameborder='NO' framespacing='0'>
    <frame src='$PHP_SELF?mode=framemenu' name='framemenu' id='framemenu' noresize scrolling='no' marginwidth='10' marginheight='0' scrollbar='no' frameborder='NO'>
<!--
    <frame src='$PHP_SELF?mode=framecont' name='framecont' id='framecont' noresize scrolling='auto' marginwidth='10' marginheight='10' scrollbar='no' frameborder='NO'>
-->
    <frame src='$cont' name='framecont' id='framecont' noresize scrolling='auto' marginwidth='10' marginheight='10' scrollbar='no' frameborder='NO'>
    <frame src='bmrace.php?mode=race-framelist' name='framerace' id='framerace' noresize scrolling='auto' marginwidth='10' marginheight='10' scrollbar='no' frameborder='NO'>
</frameset>
<noframes>
...
//			echo "<br><br><br>회원광장은 정회원과 준회원만 이용할 수 있습니다.(id='$logid')";
//			return;
			echo "
<html>
<head>
...
<frameset rows='40, *, 40' border='0' frameborder='NO' framespacing='0'>
    <frame src='$PHP_SELF?mode=framemenu_public' name='framemenu' noresize scrolling='no' marginwidth='10' marginheight='0' scrollbar='no' frameborder='NO'>
    ...
    <frame src='bmrace.php?mode=race-framelist' name='framerace' noresize scrolling='auto' marginwidth='10' marginheight='10' scrollbar='no' frameborder='NO'>
</frameset>
<noframes>
  <body bgcolor='white' text='black' link='blue' vlink='purple' alink='red'>
    <p>이 페이지를 보려면, 프레임을 볼 수 있는 브라우저가 필요합니다.</p>
...
";
		...
		echo "<br><br><br>회원광장은 정회원과 준회원만 이용할 수 있습니다.(id='$logid',error)";
		return;
	}
}
?>
