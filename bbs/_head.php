 ...
 **************************************************************************/
 	...
 	$_zb_file_list = array("rss_generator.php","apply_vote.php","comment_ok.php","del_comment.php","del_comment_ok.php","delete.php","download.php","list_all.php","view.php","vote.php","write.php","write_bmc.php","write_bmc_smarteditor.php","write_ok.php","zboard.php","image_box.php");
...
//echo("<!-- _head.member[no]=".$member[no]." -->\n");
 ...
 **************************************************************************/
	if($_zboardis) {
		...
		if($group[is_open]==0&&!$is_admin&&$member[group_no]!=$setup[group_no]){
			$errmsg="<font color=blue size='+1'><!-- yhkim 22 -->접속이 끊어졌습니다.<br>공개 되어 있지 않습니다.<br><br>입력 내용을 카피 하시든지 화면 왼쪽에서 로그인하십시오.</font>";
			// yhkim insert
			if($memo){
				echo "<script>alert(\"입력하신 자료가 날라갑니다.\n절대로 아무런 조작도 하지 마시고 '확인'을 누른 후 카피 방법 대로 처리하시기 바랍니다.\")</script>";
				$errmsg .=  "</table><br><br><br><table><tr><td><font color=red>주의 : 입력한 내용이 없어집니다. 아래 내용을 카피하시기 바랍니다.(Err 2)
<br><br>카피 방법 : <br>
 - 마우스 왼쪽 버턴을 누른 상태에서 마우스를 끌어 당겨 카피할 내용을 선택<br>
 - 마우스 오른쪽 버턴을 눌러 '복사(C)'선택<br> 
 - 메모장이나 워드에서 '붙여넣기' 실행<br>
 - 로그인 후 내용 입력 창에서 '붙여넣기' 실행</font><br><br>\n";
				$errmsg .=  "제목: $subject<br>\n";
				$errmsg .=  "내용:<br>\n";
		...
		/////////////////////////////////////////////
		// write_bmc.php가 아닐때 검색갯수 및 query 정리
		/////////////////////////////////////////////
		if(!eregi("write_bmc.php",$PHP_SELF)) {
		...
		if ($cafe_style == "yes") 
			$href="cafe_style=yes&id=$id&page=$page&sn1=$sn1&divpage=$divpage";
		else
		...
		if($dispmode) $href.="&dispmode=$dispmode";		// yhkim
...
// 회원가입버튼;;
	if(!$member[no]&&$group[use_join])  {
		$a_member_join="<a onfocus=blur() href=# onclick=\"window.open('".$_zb_url."member_join.php?group_no=$setup[group_no]','zbMemberJoin','width=560,height=590,toolbars=no,resizable=yes,scrollbars=yes')\">"; 
	}
	else  {
