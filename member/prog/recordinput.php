...
//echo "mysql_errno=".mysql_errno().":".mysql_error();
			...
			$dbquery = "update record set record='$record',item='$item',rank='$rank',etc='$lap5(5k) $lap30(30k)' where raceid=$raceid and userid='$userid'";
			$result = mysql_db_query("coretek",$dbquery);
echo "<br>update dbquery=$dbquery<p>";
	...
	echo "총 $no 건 입력 완료.";
    }else if($mode == "batch-inputgeneral"){
...
//echo "mysql_errno=".mysql_errno().":".mysql_error();
			...
			$dbquery = "update record set record='$record',item='$item',rank='$rank',etc='$etc' where raceid=$raceid and userid='$userid'";
			$result = mysql_db_query("coretek",$dbquery);
echo "<br>update dbquery=$dbquery<p>";
			if($result!=1){
				echo "<br>update 이상. dbquery=$dbquery<br>처리 미완료<p>";
