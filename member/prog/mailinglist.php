...
if($mode == "ml-input-new"){
	heading("Gumpu Mailing List 신규 등록");
	ml_display("ml-insert","","","","","","","","","","","",$logid);
}else if($mode == "ml-input-old"){
	heading("Gumpu Mailing List 삭제 요청");
...
// mail send **************************************************************
	...
	heading("Gumpu Mailing List Batch 등록");
...
// mail send **************************************************************
	...
	heading("Gumpu Mailing List 정보 삭제 요청");
	...
	$cont = "<p align=center>Gumpu Mailing List에서 귀하의 E-Mail 주소를 삭제하시려면 아래 링크를 클릭해 주십시오.\n";
	$cont .= "<br>(Please click 'Delete' button if you want to delete your email account from gumpu marathon club mailing-list)\n";
	$headers = "From: \"탄천검푸\" <gumpu@gumpu.pe.kr>\n";
//	$headers .= "X-Sender: <gumpu@gumpu.pe.kr>\n";
//	$headers .= "X-Priority: 1\n"; // Urgent message!
	$headers .= "Return-Path: <gumpu@gumpu.pe.kr>\n";  // Return path for errors
...
//	$cont.= "<p align=center><font size='+2'><a href=\"http://$HTTP_HOST$PHP_SELF?mode=ml-delete&email=".urlencode($email)."&randno=$row[1]\">삭제</a></font>\n";
	...
	$cont.= "</form>\n";
	$cont.= "</font>\n";
	if(mail($receiver, "Gumpu Mailing List 정보 삭제", $cont, $headers) == FALSE){
		echo "mail 발송 에러\n";
		die("");
	}
//echo $cont;
	...
	heading("Gumpu Mailing List 정보 삭제");
...
echo $result;
	if($result=="1"){
		...
		echo "$email 삭제 완료<br><br>나중에 탄천검푸 마라톤 클럽이나 분당마라톤에 관심을 가지시게 되면 홈페이지(www.gumpu.com)에서 다시 가입해 주시기 바랍니다.";
		echo "</td></tr></table>";
	}else{
		echo "<font color=red>$email 삭제 오류</font>";
	}
}else{
	heading("Gumpu Mailing List 메뉴");

	echo "<p><a href='$PHP_SELF?mode=ml-input-new'>Gumpu Mailing List 신규 등록</a><p>\n";
	echo "<p><a href='$PHP_SELF?mode=ml-input-old'>Gumpu Mailing List 삭제 요청</a><p>\n";
}
