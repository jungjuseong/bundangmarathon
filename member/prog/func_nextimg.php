...
arImageList[$j].alt = \"".substr($filesArray[$i],strlen("/member/racephoto"))."\";\n";
		$j++;
	}
		echo "
</SCRIPT>
<p align='center'><a href='javascript:void(0);' onClick=\"return false;\" onMouseOver=\"nextimg();\">다음 사진</a>\n";
		echo "<br><img name=racephoto src='$filesArray[0]' alt='".substr($filesArray[0],strlen("/member/racephoto"))."'>\n";
	}else if(count($filesArray) == 1){
		echo "<img src='$filesArray[0]' alt='".substr($filesArray[0],strlen("/member/racephoto"))."'>\n";
	}else{
		return(0);
	}
	return(1);
